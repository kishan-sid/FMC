...
import { Calendar, ChevronDown, MapPin, Activity, Loader2, CheckCircle2, Clock3, ArrowRight, CalendarDays } from "lucide-react";
import { fetchMatchdays, fetchMatches } from "../lib/data.js";
...
export default function Matchdays({ onOpenMatch }) {
  ...
  // Load real scraped matchdays.
  useEffect(() => {
    let alive = true;
    (async () => {
      const live = await fetchMatchdays();
      if (!alive) return;
      setMatchdays(live);
  ...
  // Load matches for the selected matchday.
  useEffect(() => {
    if (!selected) { setMatches([]); return; }
    let alive = true;
    (async () => {
      const live = await fetchMatches(selected);
      if (alive) setMatches(live);
    })();
    return () => { alive = false; };
  }, [selected]);
  ...
  if (loading) {
    return <div className="flex min-h-[40vh] items-center justify-center text-sm text-slate-500">Loading matchdays…</div>;
  }
    ...
    return (
      <div className="flex min-h-[60vh] items-center justify-center p-6">
        <div className="text-center">
          ...
          <h2 className="mt-4 text-base font-semibold text-slate-200">No matchdays yet</h2>
          <p className="mt-1 text-xs text-slate-500">Scrape a match from <span className="text-emerald-300">Scrape &amp; Export</span> to see it here.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="space-y-6 p-6">
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative">
          <select
            ...
            className="appearance-none rounded-lg border border-slate-800 bg-slate-900 pl-9 pr-9 py-2 text-sm text-slate-100 focus:border-emerald-500 focus:outline-none max-w-[480px]"
          >
            {matchdays.map((m) => (
              <option key={m.id} value={m.id}>
                {m.label}{m.date && !String(m.label).includes(m.date) ? ` · ${m.date}` : ""}
              </option>
            ))}
          </select>
      ...
      {list.length === 0 && (
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-8 text-center text-sm text-slate-500">
          No matches in this matchday yet.
        </div>
      )}
