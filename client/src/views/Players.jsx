...
import { fetchPlayers } from "../lib/data.js";
...
export default function Players() {
  ...
  const data = source ?? [];
  const teams = useMemo(() => ["ALL", ...new Set(data.map((p) => p.team))], [data]);
            ...
            <tbody className="divide-y divide-slate-800">
              {rows.length === 0 && (
                <tr><td colSpan={7} className="px-5 py-10 text-center text-sm text-slate-500">
                  No players yet — scrape a match to populate on-pitch goal analysis.
                </td></tr>
              )}
              {rows.map((p) => (
