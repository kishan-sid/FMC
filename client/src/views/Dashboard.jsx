...
import {
  fetchSummary, fetchMatchdays, fetchActivity, fetchGoalDistribution, fetchMatchdayTrend,
} from "../lib/data.js";

const EMPTY_SUMMARY = { matchesScraped: 0, playersTracked: 0, eventsParsed: 0, matchdaysScraped: 0, successRate: 0, lastRun: "—" };
const EMPTY_GOALS = ["0-15", "16-30", "31-45", "46-60", "61-75", "76-90+"].map((half) => ({ half, goals: 0 }));
...
export default function Dashboard({ onOpenMatchdays }) {
  ...
  const [goalDistribution, setGoalDistribution] = useState(EMPTY_GOALS);
  const [matchdayTrend, setMatchdayTrend] = useState([]);

  useEffect(() => {
    let alive = true;
    (async () => {
      const [s, mds, acts, dist, trend] = await Promise.all([
        fetchSummary(), fetchMatchdays(), fetchActivity(),
        fetchGoalDistribution(), fetchMatchdayTrend(),
      ]);
      if (!alive) return;
      setSummary(s || EMPTY_SUMMARY);
      setMatchdays(mds.slice(0, 5));
      setActivity(acts);
      if (dist) setGoalDistribution(dist);
      if (trend) setMatchdayTrend(trend);
    })();
    return () => { alive = false; };
  }, []);
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 rounded-xl border border-slate-800 bg-slate-900/40 p-5">
          <div className="flex items-center justify-between mb-4">
            <div>
              ...
              <p className="text-xs text-slate-400">Last 6 matchdays</p>
            </div>
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
          ...
          <p className="text-xs text-slate-400 mb-4">Across scraped matches</p>
          <div className="h-56">
            <ResponsiveContainer width="100%" height="100%">
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <div className="xl:col-span-2 rounded-xl border border-slate-800 bg-slate-900/40">
          ...
          <div className="divide-y divide-slate-800">
            {matchdays.length === 0 && (
              <div className="px-5 py-8 text-center text-xs text-slate-500">No matchdays yet — run a scrape.</div>
            )}
            {matchdays.map((md) => {
              const pct = md.matches ? Math.round((md.scraped / md.matches) * 100) : 0;
              const statusColor =
                md.status === "complete"
        ...
        <div className="rounded-xl border border-slate-800 bg-slate-900/40 p-5">
          ...
          <ul className="space-y-3">
            {activity.length === 0 && (
              <li className="py-6 text-center text-xs text-slate-500">No recent activity.</li>
            )}
            {activity.map((a) => {
