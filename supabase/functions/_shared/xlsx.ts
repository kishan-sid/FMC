// Minimal single-sheet XLSX writer (store-only ZIP, no compression, no deps).
// Shared between generate-export and scrape-tick so the per-job CSV always has
// a matching XLSX with identical rows.

export function buildSimpleXlsx(headers: string[], rows: unknown[][]): Uint8Array {
